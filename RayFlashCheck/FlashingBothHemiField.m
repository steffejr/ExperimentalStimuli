...
theImageLocation1 = '/home/jason/Dropbox/SteffenerColumbia/Scripts/ExperimentalStimuli/RayFlashCheck/medcon1.bmp';
theImageLocation2 = '/home/jason/Dropbox/SteffenerColumbia/Scripts/ExperimentalStimuli/RayFlashCheck/medcon2.bmp';
...
% Instruction Texts
% ----------------------------------------------------------------------
line1 = 'You will be seeing flashing checkerboard randomly on right or left side of the screen';
line2 = '\n and also will be hearing alternating tone randomly on one ear at a time';
line3 = '\n\n\n\n At the end of each visual stimuli press the shift key two times';
line4 = '\n Hit the right hand key twice if the visual stimuli presented on the right side';
line5 = '\n Hit the left hand key twice if the visual stimuli presented on the left side';
...
EN = 1;                           %Set the event number for auditory stimuli
VisEN = 0;                        %Set the event number for visual stimuli
...
for t=0:(ScanLength*round(1/ifi)-2)
    ...
    if (TimeLine(t+2)==0 && TimeLine(t+1)~=0)
    ...
    elseif ((TimeLine(t+2)~=0 && TimeLine(t+1)==0 && VisEN~=0) || (t==(ScanLength*round(1/ifi)-3)))  
        ...
        if pressed
            ...
            KeyBoardTiming(VisEN,1) = pressedCodes(1);
            KeyBoardTiming(VisEN,2) = firstPress(pressedCodes(1))-startTime;
            KeyBoardTiming(VisEN,3) = firstRelease(pressedCodes(1))-startTime;
            KeyBoardTiming(VisEN,4) = lastPress(pressedCodes(1))-startTime;
            KeyBoardTiming(VisEN,5) = lastRelease(pressedCodes(1))-startTime;    
...
stem([KeyBoardTiming(:,4); KeyBoardTiming(:,2)], -[(KeyBoardTiming(:,1)-227)/20;(KeyBoardTiming(:,1)-227)/20], 'r', '.')
...
filename=['HRF_BothHemiField_Visual_' subjInitials '_'  year '-' month '-' day '_' hour ':' minute];
