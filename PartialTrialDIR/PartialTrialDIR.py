...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.80.06), Thu Sep 17 17:50:07 2015
...
ITICrossHair = visual.TextStim(win=win, ori=0, name='ITICrossHair',
    ...
    depth=-7.0)
StimBrackets = visual.TextStim(win=win, ori=0, name='StimBrackets',
    text='default text',    font=u'Arial',
    pos=[0, 0.5], height=0.2, wrapWidth=None,
...
for thisTrial in trials:
    ...
    trialComponents.append(StimBrackets)
    ...
    while continueRoutine:
        ...
        elif LowerTopLine.status == STARTED and t >= (0.0 + (TrialDur+$ITIDur-win.monitorFramePeriod*0.75)): #most of one frame period left
        ...
        # *StimBrackets* updates
        if t >= 0.0 and StimBrackets.status == NOT_STARTED:
            # keep track of start time/frame for later
            StimBrackets.tStart = t  # underestimates by a little under one frame
            StimBrackets.frameNStart = frameN  # exact frame index
            StimBrackets.setAutoDraw(True)
        elif StimBrackets.status == STARTED and t >= (0.0 + (2-win.monitorFramePeriod*0.75)): #most of one frame period left
            StimBrackets.setAutoDraw(False)
        # *ISI* period
