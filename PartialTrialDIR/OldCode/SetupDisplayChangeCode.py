...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.01), Thu 24 Sep 2015 09:36:58 AM EDT
...
from numpy import sin, cos, tan, log, log10, pi, average, sqrt, std, deg2rad, rad2deg, linspace, asarray, arange
...
expName = u'SetupDisplay'  # from the Builder filename that created this script
...
UpperText = visual.TextStim(win=win, ori=0, name='UpperText',
    ...
    units=u'norm', pos=[0, 0.4], height=0.4, wrapWidth=None,
...
UpperBracketColor = 'yellow'
LowerBracketColor = 'cyan'
UpperLetColor = [0,0,0]
LowerLetColor = [0,0,0]
LetLen = 6
...
# ####################
# what if I prepare the before the trial starts????
    LetCount = -1
    for i in range(0,len(StimSet),1):
        if (StimSet[i] != '{')&(StimSet[i] !='}'):
            LetCount = LetCount + 1
            UpperText.setColor(UpperLetColor,'rgb')
            UpperText.text=(StimSet[i])
            UpperText.pos=([LetPos[LetCount],UpperTextPos])
            UpperText.draw()
        elif StimSet[i] == '{':
            UpperText.text(StimSet[i])
            UpperText.setColor(UpperBracketColor,'rgb')
            # Check to see if the left bracket is the first character
            if LetCount == -1:
                UpperText.setPos([LetPos[0]-0.1,UpperTextPos])
            else:
                UpperText.setPos([LetPos[LetCount+1]-0.1,UpperTextPos])
            UpperText.draw()
        elif StimSet[i] == '}':
#            UpperText.text(StimSet[i])
#            UpperText.setColor(UpperBracketColor,'rgb')
#            UpperText.setPos([LetPos[LetCount]+0.1,UpperTextPos])
#            UpperText.draw()
...
# #####################################################################################
        ...
        elif KeyboardResp.status == STARTED and t >= (ProbeStart + (1-win.monitorFramePeriod*0.75)): #most of one frame period left
